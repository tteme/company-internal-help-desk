...
function DepartmentForm({ department, onSubmit, onCancel, isSubmitting }) {
  ...
  function validate() {
    ...
    if (!name) {
      ...
      nextErrors.name = "Department name must not exceed 100 characters.";
    }

    if (!code) {
      nextErrors.code = "Department code is required.";
    } else if (code.length < 2 || code.length > 20) {
      nextErrors.code = "Department code must be between 2 and 20 characters.";
    ...
    if (description.length > 1000) {
      nextErrors.description =
        "Department description must not exceed 1000 characters.";
    }
  ...
  function handleSubmit(event) {
    ...
    if (isEditing) {
      departmentData.isActive = form.isActive;
    }
          ...
          <label className="flex cursor-pointer items-center gap-3 rounded-md border border-border bg-surface-muted p-3">
            ...
            <span>
              ...
              <span className="block text-xs text-text-muted">
                Inactive departments cannot be used for new request routing.
              </span>
            </span>
          </label>
        )}
